        ...
        try {
            ...
            if (hits != null && hits.length > 0) {
                ...
                if (sourceMap != null && !sourceMap.isEmpty()) {
                    Map<String, Object> truncatedSourceMap = new HashMap<>();
                    for (Map.Entry<String, Object> entry : sourceMap.entrySet()) {
                        if (entry.getValue().toString().length() > 100) {
                            truncatedSourceMap.put(entry.getKey(), "[truncated]" + entry.getValue().toString().substring(0, 100));
                        } else {
                            truncatedSourceMap.put(entry.getKey(), entry.getValue());
                        }
                    }
                    return gson.toJson(truncatedSourceMap);
                }
            }
