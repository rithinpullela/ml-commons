...
public class StreamingWrapper {
    ...
    public void sendFinalResponse(
        ...
        if (isStreaming) {
            // Send token usage BEFORE completion chunk so clients receive it while stream is open
            sendTokenUsageBatch(sessionId, parentInteractionId, tokenTracker, tenantId);
    ...
    private void sendTokenUsageBatch(String sessionId, String parentInteractionId, AgentTokenTracker tokenTracker, String tenantId) {
        ...
        try {
            ...
            // Log structured token usage per model
