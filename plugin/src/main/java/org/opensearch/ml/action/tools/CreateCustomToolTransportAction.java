...
public class CreateCustomToolTransportAction extends HandledTransportAction<ActionRequest, MLCreateCustomToolResponse> {
    ...
    private void checkNameUniqueness(MLCustomToolInput input, ActionListener<MLCreateCustomToolResponse> listener) {
        customToolsHelper.searchCustomToolByName(input.getName(), ActionListener.wrap(existing -> {
            if (existing != null) {
                listener
                    .onFailure(
                        new IllegalArgumentException("A custom tool with name '" + input.getName() + "' already exists")
                    );
                return;
            }
            // 4. Validate search template exists
            validateSearchTemplateAndIndex(input, listener);
        }, e -> {
    ...
    private void validateSearchTemplateAndIndex(MLCustomToolInput input, ActionListener<MLCreateCustomToolResponse> listener) {
        ...
        client.admin().cluster().getStoredScript(getScriptRequest, ActionListener.wrap(response -> {
            if (response.getSource() == null) {
                listener
                    .onFailure(
                        new IllegalArgumentException("Search template '" + input.getSearchTemplateName() + "' not found")
                    );
                return;
            }
                    ...
                    .whenComplete((r, throwable) -> {
                        ...
                        if (throwable != null) {
                            ...
                            try {
                                ...
                                log
                                    .info(
                                        "Custom tool created: {}, id: {}",
                                        indexResponse.getResult(),
                                        indexResponse.getId()
                                    );
                                listener.onResponse(new MLCreateCustomToolResponse(indexResponse.getId()));
                            } catch (Exception e) {
                                listener.onFailure(e);
                            }
                        }
